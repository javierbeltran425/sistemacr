  ...
  try {
    ...
    if (rol == "profesor") {
      const fieldsToInsert = materias.map(materia => 
        ({ id_usuario: newUsuario[0].id_usuario, id_materia: materia.id_materia })); 

      await knex('usuariosxmaterias').insert(fieldsToInsert)
    }
    else {
      const fieldsToInsert = materias.map(materia => 
        ({ id_usuario: newUsuario[0].id_usuario, id_materia: materia.id_materia, id_profesor: materia.id_profesor })); 
...
const editUsuario = async function (req, res) {
  ...
  try {
    ...
    const removeCurrentMaterias = await knex('usuariosxmaterias').where({id_usuario: id_usuario}).del()

    if (materias.length == 0) return res.json(editedUsuario)

    if (rol == "profesor") {
      const fieldsToInsert = materias.map(materia => 
        ({ id_usuario: id_usuario, id_materia: materia.id_materia })); 

      await knex('usuariosxmaterias').insert(fieldsToInsert)
    }
    else {
      const fieldsToInsert = materias.map(materia => 
        ({ id_usuario: id_usuario, id_materia: materia.id_materia, id_profesor: materia.id_profesor })); 
        console.log(fieldsToInsert)
