...
const Header = () => {
    ...
    const materiasRender = () => {
        return materiasLista.map((materia) => {
            return <li className='list-none pl-1 py-2 hover:bg-gray-200 cursor-pointer border-round'>{materia}</li>
        })
    }
    ...
    return (
        <div className='flex flex-column md:flex-row w-full h-2 px-6 justify-content-between align-items-center bg-gray-100 shadow-1'>
            <div>
                <Button icon="pi pi-bars" onClick={() => setVisible(true)} className="p-button-secondary p-button-outlined shadow-1" />
