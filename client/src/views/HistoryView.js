...
const HistoryView = () => {
    ...
    console.log("🚀 ~ file: HistoryView.js:15 ~ HistoryView ~ historyData:", historyData)
    ...
    const conteoSolicitudes = () => {
        if(historyData.length > 0) {
            let atendidas = historyData.filter(solicitud => solicitud.estado === 'ATENDIDO')
            let pendientes = historyData.filter(solicitud => solicitud.estado === 'PENDIENTE')
            let ausentes = historyData.filter(solicitud => solicitud.estado === 'AUSENTE')
            let rechazadas = historyData.filter(solicitud => solicitud.estado === 'RECHAZADO')
    ...
    const obtieneDatosReporte = async () => {
        try {
            ...
            console.log("🚀 ~ file: History.js:20 ~ response ~ obtieneDatosReporte:", response)
            ...
            for(let i = 0; i < history.length; i++) {
                ...
                console.log("🚀 ~ file: HistoryView.js:36 ~ obtieneDatosReporte ~ response2:", response2)
                ...
                console.log("🚀 ~ file: HistoryView.js:36 ~ obtieneDatosReporte ~ response3:", response3)
        ...
        <div className="px-6">
            ...
            <div className='flex flex-row w-full justify-content-between'>
                <div className='bg-green-500 p-4 border-round-xl shadow-3'>
                    <p className='m-0 text-white font-bold text-lg pb-2'>SOLICITUDES ATENDIDAS</p>
                    <p className='m-0 text-center text-white font-bold text-6xl'>{contAtendidas}</p>
                </div>
                <div className='bg-yellow-500 p-4 border-round-xl shadow-3'>
                    <p className='m-0 text-white font-bold text-lg pb-2'>SOLICITUDES PENDIENTES</p>
                    <p className='m-0 text-center text-white font-bold text-6xl'>{contPendientes}</p>
                </div>
                <div className='bg-orange-500 p-4 border-round-xl shadow-3'>
                    <p className='m-0 text-white font-bold text-lg pb-2'>SOLICITUDES AUSENTES</p>
                    <p className='m-0 text-center text-white font-bold text-6xl'>{contAusentes}</p>
                </div>
                <div className='bg-red-500 p-4 border-round-xl shadow-3'>
                    <p className='m-0 text-white font-bold text-lg pb-2'>SOLICITUDES RECHAZADASS</p>
                    <p className='m-0 text-center text-white font-bold text-6xl'>{contRechazadas}</p>
                </div>
            </div>
