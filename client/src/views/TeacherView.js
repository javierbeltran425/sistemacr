...
const TeacherView = () => {
    ...
    const agregarFechas = () => {
        ...
        let eventoModel = { id: Math.random() , dia: dia, inicio: nuevaFechaInicio, fin: nuevaFechaFin }
    ...
    const fechasRender = (dia) => {
        ...
        for (let i = 0; i < _aux.length; i++) {
            _fechasElementList.push(
                <div key={Math.random()+i} className='flex w-full justify-content-between align-items-center'>
                    ...
                    <Button icon="pi pi-trash" style={{ height: "1rem" }} onClick={() => deleteHorarios(_aux[i])} ></Button>
                </div>
            )
        }
    ...
    return (
        <Layout>
            <div className='w-full px-6 pt-5'>
                ...
                {/* <div className='w-full px-6 pt-2 surface-100 border-round-md shadow-1' style={{ height: "27em" }}>
                    <div className='flex justify-content-between align-items-center'>
                        <h4>Consultas generales</h4>

                        <div className='flex gap-3'>
                            <Button icon="pi pi-save" rounded text raised severity="info" aria-label="Agregar" className='mr-2' />
            ...
            <Dialog header="Definición de horarios" footer={footer} visible={visible} style={{ width: '50vw' }} onHide={() => setVisible(false)}>
                <div className='flex flex-column gap-4'>
                    ...
                    <div className='flex flex-column'>
                        ...
                        <InputMask mask='99:99' value={nuevaFechaInicio} onChange={(e) => setNuevaFechaInicio(e.value)} />
                    </div>
                    <div className='flex flex-column gap-1'>
                        <label className='font-bold'>Hora de finalización</label>
                        <InputMask mask='99:99' value={nuevaFechaFin} onChange={(e) => setNuevaFechaFin(e.value)} />
                    </div>
                    <div className='flex flex-column gap-1'>
                        <label className='font-bold'>Hora de finalización</label>
                        <Dropdown value={dia} options={dias} optionLabel='label' onChange={(e) => setDia(e.value)} />
                    </div>
                </div>
            </Dialog>
